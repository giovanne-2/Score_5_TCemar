    ...
    <div style={{ maxWidth:1440,margin:'0 auto',padding:'clamp(1rem,4vw,1.5rem) clamp(.75rem,4vw,2rem)' }}>
      ...
      {loading ? <Loader text="Carregando planilha..."/> : (
        <>
          ...
          <Card><DataTable rows={currentRows} headers={currentHeaders} fixedCols={PLANNER_FIXED_COLS} grupos={currentGrupos} extraCols={extraCols} hiddenCols={PLANNER_HIDDEN_COLS} groupsAfterCol="Mix OK" groupCellMode={aba==='he' ? 'both' : 'missing'}/></Card>
        </>
      )}
    </div>
  )
