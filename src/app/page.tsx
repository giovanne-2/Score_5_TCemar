...
export default function Planejador() {
  ...
  const currentGrupos  = aba==='he' ? GRUPOS_HE : GRUPOS_CORE
    ...
    <div style={{ maxWidth:1440,margin:'0 auto',padding:'1.5rem 2rem' }}>
      ...
      {loading ? <Loader text="Carregando planilha..."/> : (
        <>
          ...
          <Card><DataTable rows={currentRows} headers={currentHeaders} fixedCols={FIXED_COLS} grupos={currentGrupos} extraCols={extraCols}/></Card>
        </>
      )}
    </div>
  )
